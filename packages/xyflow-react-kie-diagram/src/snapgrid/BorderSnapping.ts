...
>(
  ...
  if (distanceLeft <= borderAllowanceInPx) {
    snappedX = Math.round(dropTargetX_min - shapeDimensions.width / 2); // Snap to the top side
  }
  if (distanceRight <= borderAllowanceInPx) {
    snappedX = Math.round(dropTargetX_max - shapeDimensions.width / 2); // Snap to the right side
  }
  if (distanceTop <= borderAllowanceInPx) {
    snappedY = Math.round(dropTargetY_min - shapeDimensions.height / 2); // Snap to the top side
  }
  if (distanceBottom <= borderAllowanceInPx) {
    snappedY = Math.round(dropTargetY_max - shapeDimensions.height / 2); // Snap to the bottom side
  }

  // Step 2: Leave movement free if not already snapped.
  if (distanceLeft <= borderAllowanceInPx) {
    snappedY ??= shapeBounds["@_y"]; // Snap to the top side
  }
  if (distanceRight <= borderAllowanceInPx) {
