...
export function usePotentialWaypointControls(
  ...
  const isDraggingWaypoint = useBpmnEditorStore((s) => !!s.diagram.draggingWaypoints.find((e) => e === edgeId));
  const bpmnEditorStoreApi = useBpmnEditorStoreApi();
  const reactFlowInstance = RF.useReactFlow<BpmnDiagramNodeData, BpmnDiagramEdgeData>();
  ...
  }, [bpmnEditorStoreApi, edgeIndex, isExistingWaypoint, potentialWaypoint, snappedPotentialWaypoint, waypoints]);
