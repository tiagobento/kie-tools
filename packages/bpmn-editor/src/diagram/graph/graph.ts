...
export type AdjMatrix = Record<
  string,
  undefined | Record<string, undefined | { direction: HierarchyDirection; edge: ProcessFlowEdge }>
>;
...
export type ProcessFlowAdjacencyList = Map<string, { dependencies: Set<string> }>;

export function getAdjMatrix(edges: ProcessFlowEdge[]): AdjMatrix {
...
export type EdgeVisitor = (edge: ProcessFlowEdge, traversalDirection: HierarchyDirection) => void;
...
export function buildHierarchy({ nodeId, edges }: { nodeId: string | undefined | null; edges: ProcessFlowEdge[] }) {
  if (!nodeId) {
