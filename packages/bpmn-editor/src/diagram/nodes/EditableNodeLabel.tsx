...
import { BPMN20_SPEC, UniqueNameIndex } from "@kie-tools/bpmn-marshaller/dist/schemas/bpmn-2_0/Bpmn20Spec";
...
export function EditableNodeLabel({
  ...
  const displayValue = useBpmnEditorStore((s) => {
    if (!value) {
      return undefined;
    }
  ...
  const isValid = useBpmnEditorStore((s) => {
    if (skipValidation) {
      return true;
    }

    return BPMN20_SPEC.isValidName(id ?? generateUuid(), internalValue, onGetAllUniqueNames(s));
  });
  ...
  useFocusableElement(
    ref,
    enableAutoFocusing ?? true ? id : undefined,
    useCallback(
      (cb) => {
        setTimeout(() => {
          flushSync(() => {
...
export function useEditableNodeLabel(id: string | undefined) {
  ...
  const [isEditingLabel, setEditingLabel] = useState(
    !!id &&
      !!bpmnEditorStoreApi.getState().focus.consumableId &&
      bpmnEditorStoreApi.getState().focus.consumableId === id
  );
