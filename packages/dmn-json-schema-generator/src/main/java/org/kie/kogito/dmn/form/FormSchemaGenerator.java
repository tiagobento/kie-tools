...
public class FormSchemaGenerator {
  ...
  public Form execute(final String dmnPath, final String formUrl, final String modelUrl, final String swaggerUIUrl)
    throws FileNotFoundException {
    var dmnFile = new File(dmnPath);
    var modelResource = ResourceFactory.newReaderResource(new FileReader(dmnFile), "UTF-8");
    var dmnRuntime = DMNRuntimeBuilder
      .fromDefaults()
      .buildConfiguration()
    ...
    return new Form(dmnFile.getName(), dmnModel.getName(), jsNode, formUrl, modelUrl, swaggerUIUrl);
  }
